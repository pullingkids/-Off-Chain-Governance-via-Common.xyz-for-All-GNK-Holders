...
Gonka currently uses **Native Chain Governance** on DAO DAO — a standard Cosmos SDK `x/gov` setup where only bonded (staked) GNK holders vote. In practice, this means ~100–150 validator operators control all governance decisions across every category: software upgrades, parameter changes, treasury allocations, and marketing partnerships alike.

This creates a fundamental mismatch between decision type and decision-maker:

- **Validators are infrastructure operators.** They have deep expertise in consensus security, node performance, and protocol upgrades. Giving them exclusive authority over technical decisions makes sense.
- **Validators are not the community.** Treasury allocations, marketing budgets, ecosystem grants, and partnership decisions affect all GNK holders — miners, developers, exchange buyers — not just the validator set. Excluding these stakeholders from non-technical decisions is inconsistent with Gonka's decentralization goals.

The 970,000 GNK media partnership proposal currently under vote makes this concrete: a treasury decision of this magnitude is being decided by fewer people than a typical Discord poll, with no formal input from the broader community.
...
|-------|--------------|-----------|-----------|
| Technical | Chain upgrades, parameter changes, security | Validators (bonded GNK) | Existing `x/gov` (Native DAO DAO) |
| Community | Treasury, marketing, grants, partnerships | All GNK holders | New Token-Based DAO |

Validators retain full authority over technical decisions — software upgrades, consensus parameters, node configurations. These require deep protocol knowledge and validator accountability.
...
CosmWasm is already active on Gonka mainnet. The community sale contract and liquidity pool contract confirm that the `x/wasm` module is operational. DAO DAO Token-Based DAOs are CosmWasm smart contracts — no protocol upgrade is required to deploy them.
...
```
All proposals → x/gov → Validator vote → Execution
```

### Proposed Architecture

```
Technical proposals  → x/gov (unchanged)  → Validator vote       → Execution
Community proposals  → Token-Based DAO    → All GNK holders vote → Smart contract execution
```

### Token-Based DAO Components

DAO DAO Token-Based DAOs use three CosmWasm contracts:
...
The `dao-voting-token-staked` module snapshots GNK balances at the block when a proposal is created. No staking to a validator required — holding GNK in any wallet is sufficient to vote.
...
| Security | Validator slashing, emergency halts | Technical → `x/gov` |
...
2. Deploy DAO DAO contract suite to `gonka-testnet-1` (see [#870](https://github.com/gonka-ai/gonka/discussions/870))
3. Configure `dao-voting-token-staked` against GNK token denom
4. Run test proposals on testnet; validate voting and automatic execution
...
- Validators retain full authority over technical decisions
- The existing DAO DAO interface remains for technical proposals
- Consensus, security, and upgrade decisions continue through `x/gov`
- No changes to staking mechanics, rewards, or validator economics
...
**Requested: To be determined after testnet validation**

INC4 contributes testnet deployment and initial configuration at no cost. A separate budget proposal for mainnet deployment and contract audit will be submitted once testnet scope is confirmed.

This proposal authorizes the migration plan and testnet work. No treasury spend is requested at this stage.
...
4. **Gonka Testnet** — staging environment for contract deployment and validation. Source: [gonka-ai/gonka #870](https://github.com/gonka-ai/gonka/discussions/870)
